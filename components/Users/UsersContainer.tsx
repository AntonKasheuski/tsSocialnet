...
export const UsersContainer = () => {
    const usersPageData = useSelector<AppStateType, UsersPageType>(state => state.usersPage)
    const dispatch = useDispatch()

    useEffect(() => {
        dispatch(getStartPageUsers(usersPageData.currentPage, usersPageData.pageSize))
    ...
    const onPageChanged = (pageNumber: number) => {
        dispatch(getSelectedPageUsers(pageNumber, usersPageData.pageSize))
    }

    return <>
        {usersPageData.isFetching
            ? <Preloader/>
            : <Users totalUsersCount={usersPageData.totalUsersCount}
                     pageSize={usersPageData.pageSize}
                     currentPage={usersPageData.currentPage}
                     users={usersPageData.users}
                     followingInProgressArray={usersPageData.followingInProgressArray}
